...
class Block(object):
    def __init__(self, mmap, x, y, direction=None):
        ...
        self.walls = [True, True, True, True]  # creates top,right,bottom,left walls
        self.mmap = mmap
        if mmap:
            mmap.mmap[x][y] = self
        self.x, self.y = x, y
...
class Map(object):
    ...
    def gen_map(self, max_x=10, max_y=10):
        ...
        self.solution = []
        block_stack = [Block(None, -1, -1)]  # an unused block
...
class TKDrawMap(DrawMap):
    ...
    def draw_start(self):
        ...
        start = self.w.create_rectangle(x - r, y - r, x + r, y + r, fill="blue")
        self.w.tag_bind(start, '<ButtonPress-1>', lambda e: self.draw_solution()) #clicking the button at given position executes draw_solution 

    def draw_end(self):
        r = self.cell_width // 3
        x, y = self.get_cell_center(self.mmap.max_x - 1, self.mmap.max_y - 1)
        end = self.w.create_rectangle(x - r, y - r, x + r, y + r, fill="blue")
...
def main():
    m = Map()
    m.gen_map(20, 20) #generates a random map of desired size
    TKDrawMap(m)
